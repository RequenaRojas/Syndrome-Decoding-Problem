...
G = random_generator_matrix(n, k, GF)
    ...
    resultados[k] = mejor_tiempo_seg
    print(f"✅ k={k} : Mejor tiempo = {mejor_tiempo_seg:.6f} s")

print("\n--- RESUMEN DE TIEMPOS VARIANDO K ---")
for k, tiempo in resultados.items():
    print(f"k={k}: {tiempo:.6f} segundos")
...
plt.xlabel('Dimensión de la Matriz (k)')
plt.ylabel('Tiempo de Ejecución (segundos)')
plt.text(
    x=0.05,
    y=0.90,
    s=r'$n = 1000 \quad \text{y} \quad GF(q=2)$',
...
plt.savefig('timeit_k_matriz_chequeo_paridad.svg', dpi=300) 

NOMBRE_ARCHIVO_CSV = 'timeit_k_matriz_chequeo_paridad.csv'
